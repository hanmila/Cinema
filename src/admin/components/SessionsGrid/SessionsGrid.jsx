...
import PopupFilm from '../modals/PopupFilm/PopupFilm';
import PopupSession from '../modals/PopupSession/PopupSession';
...
function SessionsGrid({ films, setFilms, halls, setHalls }) {
  ...
  // Добавление сеанса
  const handleAddSession = async ({ filmId, hallId, startTime }) => {
    try {
      const response = await api.createSeance({
        hallId,
        filmId,
        time: startTime,
      });
        ...
        if (hall.id === hallId) {
          return {
            ...
            sessions: [
              ...(hall.sessions || []),
              {
                id: response.id, // ID уже от сервера
                filmId,
                start: startTime,
              },
            ],
