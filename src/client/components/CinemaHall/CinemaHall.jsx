...
export default function CinemaHall({
  ...
  const getSeatClass = (type, isActive) => {
    ...
    if (type === "vip") return "buying-scheme__chair free_vip scheme-free";
    if (type === "taken") return "buying-scheme__chair taken";
    if (type === "disabled") return "buying-scheme__chair";
    return "buying-scheme__chair free scheme-free";
  };
  ...
  return (
    <div className={`buying-scheme ${zoomed && isMobileOrTablet ? "zoomed" : ""}`}>
      ...
      <div className="tickets-price">
        Стоимость выбранных билетов:
        <span className="payment__info-details">
          {pickedSeats.reduce((total, id) => {
            const [row, col] = id.split("_").map(Number);
            const type = layout[row]?.[col];

            return total + (type === "vip" ? prices.vip : prices.normal);
          }, 0)}
        </span>{" "} 
        рублей
      </div>
